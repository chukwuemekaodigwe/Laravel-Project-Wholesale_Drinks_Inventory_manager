...
function toBatch(batch_no) {
    router.get(`/stockByBatch/${batch_no}`)
}

function deleteStock(batch_no) {
    if (confirm('Do you really wish to delete this batch, as its irrevocable')) {
        router.delete(`/stocks/delete/${batch_no}`)
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="product in products"
                                ...
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                    <a href="#" @click="toBatch(product.batch_no)"
                                        class="font-medium  dark:text-blue-500 text-green-500">View</a>

                                </th>
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                    <a href="#" class="font-medium  dark:text-blue-500 text-blue-600"
                                        @click="router.get(`/stocks/edit/${product.batch_no}`)">Edit</a>
                                </th>

                                <td class="px-6 py-4">
                                    <a href="#" class="font-medium  dark:text-red-600 text-red-600"
                                        @click="deleteStock(product.batch_no)">Delete</a>
                                </td>
                            </tr>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="6" class="px-6 py-5 text-center bg-gray-200">
                                    ...
                                    <button class="px-2 py-2 bg-red-600 w-40 text-white rounded hover:bg-red-800 uppercase"
                                        @click="router.get(stocks.prev_page_url)">Prev</button>
                                    <button
                                        class="px-2 py-2 bg-blue-600 w-40 text-white rounded hover:bg-blue-800 uppercase"
                                        @click="router.get(stocks.next_page_url)">Next</button>
