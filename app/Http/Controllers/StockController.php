...
class StockController extends Controller
{
    ...
    public function salesReportByProduct()
    {
        ...
        // AND c.dateIn BETWEEN FROM_UNIXTIME('".$date1."') AND FROM_UNIXTIME('".$date2."')
        //  GROUP BY b.id ORDER BY b.name ASC");
        //  dd($stocks);
        
        $stocks = DB::table('stocks')
        ->groupBy('stocks.product_id')
        ->join('out_bound_stocks', 'stocks.id', '=', 'out_bound_stocks.stock_id')
        ->join('products', 'stocks.product_id', '=', 'products.id')
        
        ->whereBetween('out_bound_stocks.dateIn', [$d1, $d2])
        
        
        ->get(
            [
                'products.name',
                DB::raw('sum(out_bound_stocks.qtysold) as qtysold'),
                DB::raw('sum(out_bound_stocks.total_earning) as earning'),
                DB::raw('sum(out_bound_stocks.profit) as profit')
            ]
        );
        //dd($stocks);
        /**
         * ->sum('stocks.total_qty')
        ->sum('out_bound_stocks.qtysold')
        ->sum('out_bound_stocks.total_earning')
        ->sum('out_bound_stocks.profit')
         */
    ...
    public function salesReportByRange(Request $request)
    {
        ...
        if (!empty($request->dates)) {
            $day = $request->dates;
//return response()->json($day['to']);
            $date1 = strtotime($day['from']);
            $date2 = strtotime($day['to']);
        ...
        // AND c.dateIn BETWEEN FROM_UNIXTIME('".$date1."') AND FROM_UNIXTIME('".$date2."')
        //  GROUP BY b.id ORDER BY b.name ASC");
    //The above query gives error. Unable to find accurate result between the dates
